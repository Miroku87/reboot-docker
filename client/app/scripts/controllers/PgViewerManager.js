    ...
    return {
        ...
        abilitaIsPrerequisito: function(id_ab, lista_ab, ids) {
            ...
            for (var v in vera_lista) {
                ...
                if (
                    ...
                    (
                        pre === Constants.PREREQUISITO_SMUOVER_MP_RESET &&
                        (
                            id_ab === Constants.ID_ABILITA_SMUOVERE ||
                            id_ab === Constants.ID_ABILITA_MEDPACK_RESET
                        )
                    ) ||
                    pre === Constants.PREREQUISITO_4_SPORTIVO && n_sportivo - 1 < 4 ||
                    pre === Constants.PREREQUISITO_5_SUPPORTO_BASE && n_supp_base - 1 < 5 ||
                    pre === Constants.PREREQUISITO_3_ASSALTATA_BASE && n_assa_base - 1 < 3 ||
                    pre === Constants.PREREQUISITO_3_ASSALTATA_AVAN && n_assa_avan - 1 < 3 ||
                    pre === Constants.PREREQUISITO_3_GUASTATOR_BASE && n_guas_base - 1 < 3 ||
                    pre === Constants.PREREQUISITO_3_GUASTATO_AVAN && n_guas_avan - 1 < 3 ||
                    pre === Constants.PREREQUISITO_15_GUARDIAN_BASE && n_guar_base - 1 < 15 ||
                    pre === Constants.PREREQUISITO_5_GUARDIANO_BAAV && n_guar_base + n_guar_avan - 1 < 5 ||
                    pre === Constants.PREREQUISITO_4_ASSALTATO_BASE && n_assa_base - 1 < 4 ||
                    pre === Constants.PREREQUISITO_7_SUPPORTO_BASE && n_supp_base - 1 < 4 ||
                    pre === Constants.PREREQUISITO_15_GUARDIAN_AVAN && n_guar_avan - 1 < 15 ||
                    pre === Constants.PREREQUISITO_15_ASSALTAT_BASE && n_assa_base - 1 < 15 ||
                    pre === Constants.PREREQUISITO_15_ASSALTAT_AVAN && n_assa_avan - 1 < 15 ||
                    pre === Constants.PREREQUISITO_15_SUPPORTO_BASE && n_supp_base - 1 < 15 ||
                    pre === Constants.PREREQUISITO_15_SUPPORTO_AVAN && n_supp_avan - 1 < 15 ||
                    pre === Constants.PREREQUISITO_15_GUASTATO_BASE && n_guas_base - 1 < 15 ||
                    pre === Constants.PREREQUISITO_15_GUASTATO_AVAN && n_guas_avan - 1 < 15
                ) {
        ...
        mostraDati: function() {
            ...
            $.each(this.pg_info.abilita.civile, function(i, val) {
                ...
                tr = $("<tr style=\"display:none;\"><td colspan=\"6\">" + val.descrizione_abilita + "</td></tr>");
                $("#lista_abilita_civili").find("tbody").append(tr)
